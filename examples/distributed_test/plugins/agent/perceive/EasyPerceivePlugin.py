...
class EasyPerceivePlugin(PerceivePlugin):
    def __init__(self):
        """
        The Easy Perception plugin for the agent.
        Function include: 
    ...
    async def execute(self, current_tick: int):
        ...
        for agent in self.all_agents:
            if agent['id'] != self.agent_id:
                if math.sqrt((agent['position'][0] - self.current_position[0])**2 + (agent['position'][1] - self.current_position[1])**2) <= could_see_distance:
